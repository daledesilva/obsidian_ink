...
# iPad / Obsidian mobile / a Windows PC on the same Wi‑Fi can POST NDJSON.
#
...
# obsidian_ink/docs/debugging-lan-ingest.md).
#
...
if [[ -n "${LAN_IP}" ]]; then
	echo "LAN ingest base (iPad/Windows): http://${LAN_IP}:${LISTEN_PORT}<INGEST_PATH>" >&2
else
