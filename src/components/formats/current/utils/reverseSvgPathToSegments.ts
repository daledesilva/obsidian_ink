    ...
    switch (cmd) {
      ...
      case 'T': {
        ...
        for (let i = 0; i + 1 < p.length; i += 2) {
          ...
          if (!prevControl) {
            ...
            const control: TLDrawPoint = { x: 2 * current.x - prevControl.x, y: 2 * current.y - prevControl.y, z: DEFAULT_CONFIG.defaultZ }
            const sampled = sampleQuadratic(current, control, end, cfg.bezierSegments)
            sampled.forEach(pt => {
              const last = currentSubpath[currentSubpath.length - 1]
              if (!last || last.x !== pt.x || last.y !== pt.y) currentSubpath.push(pt)
      ...
      case 'C': {
        ...
        for (let i = 0; i + 5 < p.length; i += 6) {
          ...
          // 如果控制点或终点包含 NaN（解析异常），跳过该曲线
          if ([c1.x, c1.y, c2.x, c2.y, end.x, end.y].some(v => !isFinite(v))) {
            continue
          }

          // 检测退化为直线的情况：控制点都非常靠近起点-终点直线
          ...
          if (maxd <= DEFAULT_CONFIG.linearApproxEpsilon) {
            // 近似为直线：只插入端点（并且插值少量中间点以避免当作单点）
            const mid = { x: (current.x + end.x) / 2, y: (current.y + end.y) / 2, z: DEFAULT_CONFIG.defaultZ }
            const candidates = [mid, end]
