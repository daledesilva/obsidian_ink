...
flowchart TD
  ...
  DeviceStore --> LS[activeWindow localStorage keys prefixed au_ink_]
```
...
### 2. Device-local `localStorage` — `storage.ts` prefix `au_ink_`

- **Implementation:** [`src/logic/utils/storage.ts`](../src/logic/utils/storage.ts) — `saveLocally`, `fetchLocally`, `deleteLocally`.
- **Key format:** `au_ink_<suffix>` via [`localStorageKey()`](../src/logic/utils/storage.ts) (`LOCAL_STORAGE_PREFIX` + suffix).
- **Examples today:**
...
- **Fields:** `pluginVersion` (current Ink semver, updated on read/write), `booxConnectionEnabled` (default off), `fingerDrawingEnabled`, `strokeInputTreatAs` per editor kind, `lastDetectedStrokeInput`, `stylusSideButtonTemporaryErase` (default off), `inkCloseCount` (saved embed locks + dedicated view closes for the auto-transcribe account notice), `autoTranscribeAccountNoticeDismissed` (one-time notice suppressed on this device).
...
- **Example:** [`src/stores/device-memory-store.ts`](../src/stores/device-memory-store.ts) — `showHiddenFoldersAtom` uses `localStorageKey('show-hidden-folders')`.
