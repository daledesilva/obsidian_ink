...
Bulk vault migration (Settings → Ink migrate card) converts **both** v1 `.writing`/`.drawing` and migratable tldraw Ink SVGs; see [file-format-and-conversion.md](./file-format-and-conversion.md). There is no command-palette entry for bulk migrate — Settings and the on-open notice are the entry points. On-open migrate uses the same tldraw eligibility (`isMigratableLegacyInkFileData`) as that scan so orphan attachment SVGs and Settings stay aligned.
