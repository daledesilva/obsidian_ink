...
function buildWritingEmbed(filepath: string): string {
	return `\n ![InkWriting](<${filepath}>) [Edit Writing](${INK_BASE_URL}?type=inkWriting&pendingPaste=true)\n`;
}

function buildDrawingEmbed(filepath: string): string {
	return `\n ![InkDrawing](<${filepath}>) [Edit Drawing](${INK_BASE_URL}?type=inkDrawing&width=500&aspectRatio=1.778&viewBoxX=0&viewBoxY=0&viewBoxWidth=500&viewBoxHeight=281&pendingPaste=true)\n`;
}
...
describe("Embed Copy-Paste Paths — relative path (scenario-independent)", function () {
	...
	it("relative path — pasted into different folder shows not-found (path wrong in new context)", async function () {
		const relativeEmbed = `\n ![InkWriting](<../Ink/Writing/hello-world.svg>) [Edit Writing](${INK_BASE_URL}?type=inkWriting&pendingPaste=true)\n`;
