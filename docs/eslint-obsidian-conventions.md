...
- **Deprecated Obsidian APIs (1.13+)** — Replace `ButtonComponent.setWarning()` with `setDestructive()` for destructive confirm actions; use `Notice.messageEl` instead of `noticeEl`. Do not call deprecated `PluginSettingTab.display()` from plugin code paths when a private render helper can do the same work.
