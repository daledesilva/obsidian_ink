...
Ink keeps a small set of intentional exceptions. Pen scroll-lock teardown still uses literal `style.overflow` / `scrollbarColor` writes (to avoid an unpin flash); those stay in code with intent comments and **without** `eslint-disable` directives, because the community scanner forbids disabling `obsidianmd/no-static-styles-assignment`. Product-name UI strings (Ink, Boox, SVG) may warn under `ui/sentence-case` rather than using disables for every notice.
...
| `no-static-styles-assignment` | Static layout lives in SCSS. Pen scroll-lock unpin keeps literal `style.overflow` / `scrollbarColor` writes with intent comments only — no eslint-disable (see gotchas). |
...
- **Pen scroll-lock must stay literal inline** — Unpinning `.ink-cm-scroller--scroll-pinned` restores `overflow` / `scrollbarColor` via literal assignments “kept inline to avoid flash on unpin.” Intent comments stay; eslint-disable comments must not. Do not replace those writes with CSS-only teardown or variable-assignment lint tricks without re-testing flash on device. Expect the static-styles rule to still flag those lines — that is accepted intentional noise for the community scan.
